...
const GetMoodPage = () => {
  ...
  const [selectedEmotion, setSelectedEmotion] = useState<'laugh'|'smile'|'meh'|'frown'|'angry'|null>(null);
  const [thoughts, setThoughts] = useState<string>("");

  const emotions: Array<'laugh'|'smile'|'meh'|'frown'|'angry'> = ['angry','frown','meh','smile','laugh'];

  const handleEmojiSelect = (emotion: 'laugh'|'smile'|'meh'|'frown'|'angry') => {
    ...
    <SafeAreaView>
      ...
      <ScrollView>
        ...
        <Card>
          ...
          <CardContent>
            <Text>{/* Fetch and display communication tips from backend here */}</Text>
          </CardContent>
        </Card>

        <Text className="text-2xl font-semibold text-gray-900 mt-8">How do you think Maya's been feeling?</Text>
        <View className='flex-row justify-center mt-5'>
          {emotions.map((emotion) => (
            <Button
              key={emotion} 
              onPress={() => handleEmojiSelect(emotion)} 
              className={`ml-1 p-6 justify-center items-center w-20 h-20${selectedEmotion === emotion ? 'border border-blue-500' : ''}`}
              variant="outline"
            >
        ...
        <Text className="text-2xl font-semibold text-gray-900 mt-8">Do you have any thoughts?</Text>
        <Input 
          value={thoughts} 
